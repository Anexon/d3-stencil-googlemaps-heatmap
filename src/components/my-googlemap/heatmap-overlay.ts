...
export class HeatmapOverlay{
    ...
    onAdd(googleMap, overlay){
        ...
        let ne = this.mapBounds.getNorthEast(),
            sw = this.mapBounds.getSouthWest(),
            mapX1 = sw.lng(),
            mapX2 = ne.lng(),
            mapY1 = ne.lat(),
            kmXStep = this.getDistance(new google.maps.LatLng(mapX1, mapY1),new google.maps.LatLng(mapX2,mapY1))/this.googleMap.getDiv().offsetWidth;
    ...
    public draw(){
        ...
        this.canvasContext.clearRect(-this.googleMap.getDiv().offsetWidth / 2, -this.googleMap.getDiv().offsetHeight / 2, 
        this.googleMap.getDiv().offsetWidth, this.googleMap.getDiv().offsetHeight);

        this.drawCircles(this.canvasContext, this.brushes, projection);
        let image = this.canvasContext.getImageData(0, 0, 
            this.googleMap.getDiv().offsetWidth, this.googleMap.getDiv().offsetHeight);
    ...
    private colorize(pixels, gradient){
    ...
    private drawCircles(canvasContext, projection, brushes){
    ...
    private generateBrushes(allRadius, normalizer){
        let brushes: any = {};
