...
const myGooglemapCss = ".map{width:1200px;height:600px}";
...
const AppHome = class {
    ...
    async componentDidRender() {
        await gmapsInit("AIzaSyAjigXRm-xmYiNyOoLMB2p1UmtauinyAy0");
        ...
        overlay.onAdd = () => {
            ...
            let ne = this.originalBounds.getNorthEast(), sw = this.originalBounds.getSouthWest(), mapX1 = sw.lng(), mapX2 = ne.lng(), mapY1 = ne.lat(), kmXStep = this.getDistance(new google.maps.LatLng(mapX1, mapY1), new google.maps.LatLng(mapX2, mapY1)) / this.map.getDiv().offsetWidth, brushes = this.generateBrushes(map(this.pointsLocation, function (d) { return Math.round(d.radius); }).keys(), kmXStep);
            overlay.draw = () => {
                this.projection = overlay.getProjection();
                ctx.save();
                ctx.clearRect(-this.map.getDiv().offsetWidth / 2, -this.map.getDiv().offsetHeight / 2, this.map.getDiv().offsetWidth, this.map.getDiv().offsetHeight);
                this.drawCircles(ctx, brushes);
                let image = ctx.getImageData(0, 0, this.map.getDiv().offsetWidth, this.map.getDiv().offsetHeight);
    ...
    render() {
        return (index.h("div", null, index.h("div", { class: "map" })));
    }
    get el() { return index.getElement(this); }
};
AppHome.style = myGooglemapCss;

exports.my_googlemap = AppHome;
